...
typedef struct {
    Vector3 pos;
    Color faces[6];
    int ix, iy, iz;   // grid coordinates 0..RUBIKS_SIZE-1
} Cubelet_r;
...
typedef struct {
    ...
    float* observations; // Required. Must match OBS_TENSOR_T in binding.c
    float* actions; // Required. int* for discrete/multidiscrete, float* for box
    float* rewards; // Required
    float* terminals; // Required. We don't yet have truncations as standard
    unsigned int rng;
    int num_agents;
    ...
    int *tmp;
    int *r_tmp;
    int render_stickers[6 * RUBIKS_SIZE * RUBIKS_SIZE];
    int render_result_stickers[6 * RUBIKS_SIZE * RUBIKS_SIZE];
...
#define OBS(env,f,r,c,color) \
        ((env)->observations[((f)*RUBIKS_SIZE*RUBIKS_SIZE*6) + ((r)*RUBIKS_SIZE*6) + ((c)*6) + (color)])
#define STICKER(env,f,r,c) ((env)->stickers[(f)*RUBIKS_SIZE*RUBIKS_SIZE + (r)*RUBIKS_SIZE + (c)])

#define R_TMP(i,j) (env)->r_tmp[(i)*RUBIKS_SIZE + (j)]
...
void precompute_strips(Cube *env) {
    ...
    // FRONT (F):
    env->strips[F][0] = (strip_t){U, RUBIKS_SIZE-1, 0,   0,  1};
    env->strips[F][1] = (strip_t){R, 0,   0,   1,  0};
    env->strips[F][2] = (strip_t){D, 0,   RUBIKS_SIZE-1, 0, -1};
    env->strips[F][3] = (strip_t){L, RUBIKS_SIZE-1, RUBIKS_SIZE-1,-1,  0};
    // BACK (B):
    env->strips[B][0] = (strip_t){U, 0,    RUBIKS_SIZE-1, 0, -1};
    env->strips[B][1] = (strip_t){L, 0,    0,   1,  0};
    env->strips[B][2] = (strip_t){D, RUBIKS_SIZE-1,  0,   0,  1};
    env->strips[B][3] = (strip_t){R, RUBIKS_SIZE-1,  RUBIKS_SIZE-1, -1, 0};
    // UP face
    ...
    // DOWN (D):
    env->strips[D][0] = (strip_t){F, RUBIKS_SIZE-1, 0, 0, 1};
    env->strips[D][1] = (strip_t){L, RUBIKS_SIZE-1, 0, 0, 1};
    env->strips[D][2] = (strip_t){B, RUBIKS_SIZE-1, 0, 0, 1};
    env->strips[D][3] = (strip_t){R, RUBIKS_SIZE-1, 0, 0, 1};

    // RIGHT (R):
    env->strips[R][0] = (strip_t){U, 0,   RUBIKS_SIZE-1, 1, 0};
    env->strips[R][1] = (strip_t){B, RUBIKS_SIZE-1, 0,  -1,0};
    env->strips[R][2] = (strip_t){D, 0,   RUBIKS_SIZE-1, 1, 0};
    env->strips[R][3] = (strip_t){F, 0,   RUBIKS_SIZE-1, 1, 0};

    // LEFT (L):
    env->strips[L][0] = (strip_t){U, 0,   0, 1, 0};
    env->strips[L][1] = (strip_t){F, 0,   0, 1, 0};
    env->strips[L][2] = (strip_t){D, 0,   0, 1, 0};
    env->strips[L][3] = (strip_t){B, RUBIKS_SIZE-1, RUBIKS_SIZE-1,-1,0};
}

//Main init
void init(Cube* env) {
    env->stickers = malloc(6 * RUBIKS_SIZE * RUBIKS_SIZE * sizeof(int));
    ...
    env->anim_time = 0.5;
    env->tmp = malloc(RUBIKS_SIZE * sizeof(int));
    env->r_tmp = malloc(RUBIKS_SIZE * RUBIKS_SIZE * sizeof(int));
}

void reset_stickers(Cube* env) {
    for(int i = 0; i < 6; i++) {
        int col = i;
            for(int j = 0; j < RUBIKS_SIZE; j++) {
               for(int k = 0; k < RUBIKS_SIZE; k++) {
...
void compute_observations(Cube* env) {
    for (int f=0; f<6; f++) { //face
        for (int r=0; r<RUBIKS_SIZE; r++) { //row
            for (int c=0; c<RUBIKS_SIZE; c++) { //col
...
static void rotate_face_ccw(Cube *env, int f) {
    for (int i=0;i<RUBIKS_SIZE;i++)
        for (int j=0;j<RUBIKS_SIZE;j++)
            R_TMP(RUBIKS_SIZE-1-j,i) = STICKER(env,f,i,j);
    for (int i=0;i<RUBIKS_SIZE;i++)
        for (int j=0;j<RUBIKS_SIZE;j++)
...
static void rotate_face(Cube *env, int f) {
    for (int i=0;i<RUBIKS_SIZE;i++)
        for (int j=0;j<RUBIKS_SIZE;j++)
            R_TMP(j,RUBIKS_SIZE-1-i) = STICKER(env,f,i,j);
    for (int i=0;i<RUBIKS_SIZE;i++)
        for (int j=0;j<RUBIKS_SIZE;j++)
            STICKER(env,f,i,j) = R_TMP(i,j);
}
...
// Centre stickers do not move, so each face's solved colour matches its face index.
int is_solved(Cube *env) {
    for (int f = 0; f < 6; f++) {
        int color = f;
        for (int r = 0; r < RUBIKS_SIZE; r++) {
            for (int c = 0; c < RUBIKS_SIZE; c++) {
...
static inline int in_layer(Vector3 pos, int axis, int layer) {
    float half = (RUBIKS_SIZE - 1) / 2.0f;
...
void c_render(Cube* env) {
    ...
    if (env->pending_animation && !playing_animation) {
        ...
        anim.rotating = 1;
        anim.axis = FACE_AXIS[env->animation_face];
        anim.layer = FACE_LAYER[env->animation_face] ? RUBIKS_SIZE - 1 : 0;
        anim.dir = FACE_SIGN[env->animation_face] * env->animation_turns;
        anim.elapsed = 0.0f;
        anim.duration = env->anim_time;

        env->stickers = env->render_stickers;
        env->pending_animation = 0;
        playing_animation = 1;
        while (anim.elapsed < anim.duration && !WindowShouldClose()) {
            anim.elapsed += GetFrameTime();
            c_render(env);
        }
        playing_animation = 0;
        anim.rotating = 0;
        env->stickers = env->render_result_stickers;

        if (env->animation_solved) {
            float elapsed = 0.0f;
            playing_animation = 1;
            while (elapsed < RUBIKS_SOLVED_PAUSE && !WindowShouldClose()) {
                elapsed += GetFrameTime();
                c_render(env);
            }
            playing_animation = 0;
            env->animation_solved = 0;
        }
    }
    ...
    //CUBE
    for (int x=0; x<RUBIKS_SIZE; x++) {
        for (int y=0; y<RUBIKS_SIZE; y++) {
            for (int z=0; z<RUBIKS_SIZE; z++) {
                Vector3 pos = (Vector3){
                    (x-half)*spacing,
                    (y-half)*spacing,
                    (z-half)*spacing
                ...
                // Right (+X)
                if (x == RUBIKS_SIZE - 1)
                    faces[0] = sticker_colors[ STICKER(env, R, RUBIKS_SIZE - 1 - y, RUBIKS_SIZE - 1 - z) ];
                // Left (−X)
                if (x == 0)
                    faces[1] = sticker_colors[ STICKER(env, L, RUBIKS_SIZE - 1 - y, z) ];
                // Up (+Y)
                if (y == RUBIKS_SIZE - 1)
                    faces[2] = sticker_colors[ STICKER(env, U, z, x) ];
                // Down (−Y)
                if (y == 0)
                    faces[3] = sticker_colors[ STICKER(env, D, RUBIKS_SIZE-1-z, x) ];
                // Front (+Z)
                if (z == RUBIKS_SIZE - 1)
                    faces[4] = sticker_colors[ STICKER(env, F, RUBIKS_SIZE - 1 - y, x) ];
                // Back (−Z)
                if (z == 0)
                    faces[5] = sticker_colors[ STICKER(env, B, RUBIKS_SIZE - 1 - y, RUBIKS_SIZE - 1 - x) ];
                rlPushMatrix();
                // rotate only the turning layer while animating
                if (anim.rotating && in_layer(pos, anim.axis, anim.layer)) {
    ...
    if (env->user_mode){
        ...
        if (IsKeyPressed(KEY_RIGHT) || IsKeyPressed(KEY_LEFT)) {
            env->highlight_layer = (env->highlight_layer == 0) ? RUBIKS_SIZE - 1 : 0;
        ...
        float extent = (RUBIKS_SIZE*spacing)/2.0f + 0.1f;
    ...
    if (current_stickers != NULL) {
        env->stickers = current_stickers;
    }
}
...
void c_step(Cube* env) {
    ...
    if (env->render) {
        memcpy(env->render_result_stickers, env->stickers,
            sizeof(env->render_result_stickers));
        env->animation_solved = solved;
    }
