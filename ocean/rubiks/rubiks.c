...
static inline int axis_layer_to_face(int axis, int layer) {
    int outer = (layer == RUBIKS_SIZE-1); // 1 if the positive side slab
    switch (axis) {
...
static inline int highlight_to_action(const Cube *env, int cw) {
    int face = axis_layer_to_face(env->highlight_axis, env->highlight_layer);
...
int main() {
    ...
    env.observations = calloc(RUBIKS_OBS_SIZE, sizeof(float));
